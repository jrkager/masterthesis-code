...
def generate_testset(d, *, perm=True, flip=True):
    """
    ...
    for k in range(4, 2 ** (d - 1) + 1):
        ...
        for ws in W[k - 1]:
            # ws is a set (of tuples)
            for v in nghbrs(ws):
                w = ws.union([v])
                hndiff = hypercube_nodes.difference(w)

                if not eq_contained(w, H):
                    H.add(frozenset(w))
...
if __name__ == "__main__":
    try:
        ...
        print("Provide dimension as command line arg. (e.g. `python bestepsilon_memeff.py 3`")
    else:
